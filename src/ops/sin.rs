define_opencl_fw_x_impl!(SinFwImpl, sin_fw_kernel);
define_opencl_bw_x_impl!(SinBwImpl, sin_bw_kernel);
...
mod tests {
    ...
    fn check_sin_fw() {
        ...
        y.alloc();
        dev.call_fw_impl("sin_fw_impl", &[&x], &[], &[], &mut [&mut y]);
    ...
    fn check_sin_bw() {
        let y_f = |x: f64| x.sin();
        let gx_f = |x: f64, _y: f64, gy: f64| 1. + gy * x.cos();
        ...
        dev.call_bw_impl("sin_bw_impl", &[&x], &[&y], &[&gy], &[], &[], &mut gx);
