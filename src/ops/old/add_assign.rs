...
pub struct AddAssignImpl {
    kernel: Mutex<Kernel>,
    wgs_x: u32,
}

impl AddAssignImpl {
    pub fn new(program: &Program, queue: Queue) -> AddAssignImpl {
        let device = queue.device();
        let kernel = Kernel::builder()
            .program(program)
            .name("add_assign_kernel")
            .queue(queue)
            .arg(None::<&Buffer<f32>>)
            .arg(0)
            .arg(0)
        ...
        match kernel.wg_info(device, CompileWorkGroupSize).unwrap() {
            CompileWorkGroupSizeResult([wgs_x, _, _]) => AddAssignImpl {
...
mod tests {
    ...
    fn add_assign_test() {
        ...
        for tc in &test_cases {
            ...
            dev.call_fw_impl("add_assign_impl", &[&x], &[], &[], &mut [&mut y]);
