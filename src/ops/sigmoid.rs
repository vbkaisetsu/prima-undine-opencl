define_opencl_fw_x_impl!(SigmoidFwImpl, sigmoid_fw_kernel);
define_opencl_bw_x_impl!(SigmoidBwImpl, sigmoid_bw_kernel);
...
mod tests {
    ...
    #[test]
    fn check_sigmoid_fw() {
        let y_f = |x: f64| 0.5 + 0.5 * (0.5 * x).tanh();
        ...
        y.alloc();
        dev.call_fw_impl("sigmoid_fw_impl", &[&x], &[], &[], &mut [&mut y]);
    ...
    fn check_sigmoid_bw() {
        let y_f = |x: f64| 0.5 + 0.5 * (0.5 * x).tanh();
        let gx_f = |_x: f64, y: f64, gy: f64| 1. + gy * y * (1. - y);
        ...
        dev.call_bw_impl("sigmoid_bw_impl", &[&x], &[&y], &[&gy], &[], &[], &mut gx);
