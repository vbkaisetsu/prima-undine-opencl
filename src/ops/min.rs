...
pub struct MinFwImpl {
    kernels: Vec<Mutex<Kernel>>,
    internal: Arc<crate::OpenCLInternal>,
}

impl MinFwImpl {
    pub fn new(program: &Program, internal: &Arc<crate::OpenCLInternal>) -> MinFwImpl {
        let kernels = (0..=10)
            .map(|i| {
                Mutex::new(
                    ocl_core::create_kernel(
                        program,
                        "min_fw_kernel_".to_string() + &(1 << i).to_string(),
                    )
                    .unwrap(),
                )
            })
...
pub struct MinBwImpl {
    kernels: Vec<Mutex<Kernel>>,
    internal: Arc<crate::OpenCLInternal>,
}

impl MinBwImpl {
    pub fn new(program: &Program, internal: &Arc<crate::OpenCLInternal>) -> MinBwImpl {
        let kernels = (0..=10)
            .map(|i| {
                Mutex::new(
                    ocl_core::create_kernel(
                        program,
                        "min_bw_kernel_".to_string() + &(1 << i).to_string(),
                    )
                    .unwrap(),
                )
            })
